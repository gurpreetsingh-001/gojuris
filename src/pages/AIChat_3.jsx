// src/pages/AIChat_3.jsx
import React, { useState, useEffect, useRef } from 'react';
import Sidebar from '../components/Sidebar';
import ApiService from '../services/apiService';
import { Link, useNavigate } from 'react-router-dom';
import { MoreVertical, Menu, Plus, X, Send, Mic, ChevronDown, Copy, Printer, MessageSquare, FileText, Bot, Receipt } from "lucide-react";
...
const AIChat_3 = () => {
  ...
  // All functions (same as before - I'll keep them identical)
  ...
  return (
    <div className="redesigned-layout">
      ...
      <div className="redesigned-main">
        {/* Top Navigation - USING OLD HEADER STYLE */}
        <header className="redesigned-header">
          ...
          <div className="header-right">
            {/* OLD STYLE ICON BUTTONS */}
            <button className="icon-btn-old" onClick={() => navigate('/dashboard')} title="Dashboard">
              <img src="/Images/home.png" alt="Dashboard" />
            </button>
            <button className="icon-btn-old" onClick={() => navigate('/Latest-Law')} title="Latest Law">
              <img src="/Images/Latest-Law.png" alt="Latest Law" />
            </button>
            <button className="icon-btn-old" onClick={() => navigate('/SaveBookmarks')} title="Bookmarks">
              <img src="/bookmark.png" alt="Bookmarks" />
            </button>
            <button className="icon-btn-settings" onClick={() => setShowSettingsModal(true)} title="Settings">
              <i className="bx bx-cog"></i>
            </button>

            <div className="user-dropdown-container" ref={dropdownRef}>
              <button className="user-button-old" onClick={() => setShowAccountDropdown(!showAccountDropdown)}>
              ...
              {showAccountDropdown && (
                <div className="user-dropdown-old">
                  <div className="dropdown-header-old">
                    <div className="dropdown-avatar-old">{getUserInitials()}</div>
                    <div className="dropdown-info-old">
                      <div className="dropdown-name-old">{userProfile?.displayName || userProfile?.name || 'Legal User'}</div>
                      <div className="dropdown-email-old">{userProfile?.email || 'user@gojuris.com'}</div>
                    </div>
                  </div>
                  <div className="dropdown-divider-old"></div>
                  <button className="dropdown-link-old">
                    <i className="bx bx-user-circle"></i>View Profile
                  </button>
                  <button className="dropdown-link-old">
                    <i className="bx bx-credit-card"></i>Billing & Plans
                  </button>
                  <button className="dropdown-link-old">
                    <i className="bx bx-history"></i>Search History
                  </button>
                  <div className="dropdown-divider-old"></div>
                  <button className="dropdown-link-old danger-old" onClick={handleSignOut}>
          ...
          <div className="messages-container">
            {chatHistory.length === 0 ? (
              <div className="welcome-screen">
                {/* OLD STYLE MODE BUTTONS */}
                <div className="mode-selector-old">
                  <button
                    onClick={() => {
                      setChatHistory([]);
                      setChatsessionId(null);
                    ...
                    className={`mode-btn-old ${chatType === 'AISearch' ? 'active-old' : ''}`}
                  ...
                  <button
                    ...
                    className={`mode-btn-old ${chatType === 'AIHelp' ? 'active-old' : ''}`}
                    title="Ask anything—get instant case laws, sections, and legal answers."
                  >
                  ...
                  <button
                    ...
                    className={`mode-btn-old ${chatType === 'Draft' ? 'active-old' : ''}`}
                  ...
                  <label htmlFor="fileUploadNew" className="mode-btn-old upload-btn-old">
                ...
                {/* OLD STYLE EXAMPLES */}
                <div className="examples-section-old">
                  <h3>Examples</h3>
                  <div className="examples-list-old">
                    {(chatType === 'AISearch' ? quickQuestions : chatType === 'AIHelp' ? helpExample : draftExample).map((question, index) => (
                      <button
                        key={index}
                        onClick={() => handleQuickQuestion(question)}
                        disabled={isLoading}
                        className="example-item-old"
                      >
                        <span className="example-number-old">{index + 1}.</span>
                        <span className="example-text-old">{question}</span>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="coming-soon-content">
                <i className="bx bx-time-five coming-soon-icon"></i>
                <h3>Coming Soon</h3>
    ...
    <style jsx>{`
        ...
        .dropdown-link-old.danger-old {
          color: #ef4444;
        }

        .dropdown-link-old.danger-old:hover {
          background: #fef2f2;
        }

        /* ============================================
           CHAT CONTENT - SCROLLABLE WITH PROPER SPACING
           ============================================ */
        .redesigned-content {
          flex: 1;
          overflow-y: auto;
        ...
        .messages-container {
          flex: 1;
          padding: 24px 32px;
          padding-bottom: 250px; /* Increased to ensure input section is always visible */
          overflow-y: auto;
        }
        ...
        .upload-btn-old:hover {
          border-color: #8B5CF6;
        }

        /* OLD STYLE EXAMPLES */
        .examples-section-old {
          margin-top: 24px;
          padding-bottom: 40px; /* Extra padding for mobile */
        }

        .examples-section-old h3 {
          font-size: 18px;
          font-weight: 600;
          color: #1a1a1a;
          margin: 0 0 16px 0;
        ...
        .example-item-old:hover:not(:disabled) {
          color: #7C3AED;
        }

        .example-item-old:disabled {
          opacity: 0.6;
          cursor: not-allowed;
        }
        ...
        /* iPad Specific (for better compatibility) */
        @media only screen 
        ...
        /* Landscape Mode - Extra small height */
