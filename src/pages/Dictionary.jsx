...
const Dictionary = () => {
  ...
  const loadCourts = async () => {
    try {
      setSelectedCourt('A vinculo matrimonii');
       const apiResponse = await ApiService.getDics();
      setBlogPosts(apiResponse || []);
      loadJudgements('A vinculo matrimonii');
  ...
  const loadJudgements = async (courtKey) => {
    ...
    const apiResponse = await ApiService.getDefination(courtKey);
    setdefination(apiResponse || []);
        ...
        <div className="database-container">
          ...
          <div className="content-wrapper">
            ...
            <div className="courts-sidebar">
              <div className="courts-header">Select Word</div>
              ...
              {isLoadingCourts ? (
                <div className="loading-state">Loading Dictionary...</div>
              ) : courtsError ? (
                <div className="error-state">{courtsError}</div>
              ) : (
                <div className="courts-list">
                  {filteredItems.map((state, idx) => (
                    <div
                      key={idx}
                      className={`court-item ${selectedCourt === state ? 'active' : ''}`}
                      onClick={() => handleCourtSelect(state)}
                    >
                      {state}
                    </div>
                  ))}
